...
Cox_generate_data = function(n, m0 = 40, k=2, beta) {
  ...
  S = c(-log(runif(2*m0,0,1))/(exp(cbind(t,(1-t),X,X*t,Z)%*%beta)[1:(2*m0)]),rep(NA, n - 2 * m0))
  C=runif(n,0,2)
  E=runif(2 * m0,0,1)
...
list_summary_Discrete=function(list){
  ...
  P1=function(dat){sum(dat[,"X"]==1&dat[,"t"]==1)/sum(dat[,"X"]==1)}
  Pm1=function(dat){sum(dat[,"X"]==-1&dat[,"t"]==0)/sum(dat[,"X"]==-1)}
...
```{r}
#r=81
#data=Cox_generate_data(n=400,m0=20,k=2,beta=c(0.5,0.5,1, -1, 0.5, 0.5))
#m0=40
#c=1
#Consider Delayed response with arrival time interval e^{-0.1}
COX_CR=function(data,m0=40,beta){
   ...
   for (c in 1:((nrow(data)-2*m0)/10)){
    ...
    new_hazard=exp(cbind(new_patient[,"t"],(1-new_patient[,"t"]),new_patient[,"X"],new_patient[,"X"]*new_patient[,"t"],new_patient[,"Z1"],new_patient[,"Z2"])%*%beta)
    data[(2*m0+c*10-9):(2*m0+c*10),"S"]=-log(runif(10,0,1))/new_hazard
...
COX_CAR=function(data,m0=40,beta){
  cov=data[,1:3]
  CARres=HuHuCAR(cov)
  data[,"t"]=ifelse(CARres$assignments=="A",1,0)
   ...
   for (c in 1:((nrow(data)-2*m0)/10)){
    ...
    new_patient=data.frame(data[(2*m0+c*10-9):(2*m0+c*10),])
    new_hazard=exp(cbind(new_patient[,"t"],(1-new_patient[,"t"]),new_patient[,"X"],new_patient[,"X"]*new_patient[,"t"],new_patient[,"Z1"],new_patient[,"Z2"])%*%beta)
    data[(2*m0+c*10-9):(2*m0+c*10),"S"]=-log(runif(10))/new_hazard
    data[(2*m0+c*10-9):(2*m0+c*10),"E"] =ifelse(data[(2*m0+c*10-9):(2*m0+c*10),"S"]<data[(2*m0+c*10-9):(2*m0+c*10),"C"],1,0)
...
Cox_CARA=function(data,m0 = 40 ,beta){
  for (c in 1:((nrow(data)-2*m0)/10)){
    data[(2*m0+(c*10)-9):(2*m0+(c*10)),"Cohort"]=rep(c,10)
    data[1:(2*m0+(c-1)*10),"A"]=data[1:(2*m0+(c-1)*10),"A"]+rexp(1,rate=1)
    ...
    cox_fit = coxph(Surv(pmin(A,S,C)) ~ t+X+t:X,data=data_before)
    #cox_fit = coxph(Surv(S,E) ~ t+X+t:X,data=data_before)
    new_patient=data.frame(data[(2*m0+c*10-9):(2*m0+c*10),c("X","t","Z1","Z2")])
    ...
    surv1 <- survfit(cox_fit, newdata = new_patient1)
    surv0 <- survfit(cox_fit, newdata = new_patient0)
    
    median_followup=median(pmin(data_before[,"A"],data_before[,"S"],data_before[,"C"]))
    #median_followup=median(data_before[,"S"])
    #median_followup=1
    
    surv_prob1 <- summary(surv1, times = median_followup)$surv
    surv_prob0 <- summary(surv0, times = median_followup)$surv
    
    hazard1=exp(predict(cox_fit, newdata = new_patient1, type = "lp"))
    hazard0=exp(predict(cox_fit, newdata = new_patient0, type = "lp"))
    
    rho=surv_prob1 / (surv_prob1 + surv_prob0)
...
#c=1
#m0=40
#data=Cox_generate_data(n=400,k = 2,beta=c(0.5,0.5,1, -1, 0.5, 0.5))
#r=81
#r=2*m0+c*10-9
New_Cox_CARA=function(data,m0 = 40 ,k=2,beta,omega=rep(0.25,4),p=0.8){
   for (c in 1:((nrow(data)-2*m0)/10)){
    data[(2*m0+(c*10)-9):(2*m0+(c*10)),"Cohort"]=rep(c,10)
    data[1:(2*m0+(c-1)*10),"A"]=data[1:(2*m0+(c-1)*10),"A"]+rexp(1,rate=1)
    data_before=data.frame(data[1:(2*m0+(c-1)*10),])
    ...
    cox_fit_Xn=coxph(Surv(pmin(A,S,C)) ~ t,data=data_before_Xn)

    #cox_fit_Xn=coxph(Surv(S,E) ~ t,data=data_before_Xn)
    #summary(cox_fit_Xn)
    ...
    new_patient1=data.frame(data_new1[nrow(data_new1),c("X","t")])
    new_patient0=data.frame(data_new0[nrow(data_new0),c("X","t")])
    
    surv1 <- survfit(cox_fit_Xn, newdata = new_patient1)
    surv0 <- survfit(cox_fit_Xn, newdata = new_patient0)
    
     median_followup=median(pmin(data_before_Xn[,"A"],data_before_Xn[,"S"],data_before_Xn[,"C"]))
    #median_followup=median(data_before_Xn[,"S"])
    #median_followup=1
...
CR_pts_list <- foreach(i = 1:1000, .export = ls(envir = globalenv()),.packages = "survival") %dopar% {
  COX_CR(
...
CAR_pts_list <- foreach(i = 1:1000, .export = ls(envir = globalenv()),.packages = c("survival","carat")) %dopar% {
  COX_CAR(
    data = Cox_generate_data(
      400,
      k = 2,
...
cox_pts_list <- foreach(i = 1:1000, .export = ls(envir = globalenv()),.packages = "survival") %dopar% {
  Cox_CARA(
    data = Cox_generate_data(
      400,
      k = 2,
...
new_cox_pts_list <- foreach(i = 1:1000, .errorhandling = 'pass', .export = ls(envir = globalenv()),.packages = "survival") %dopar% {
  New_Cox_CARA(
    data = Cox_generate_data(
      400,
      k = 2,
...
```{r}
COX_res=data.frame(
list_summary_Discrete(CR_pts_list),
list_summary_Discrete(CAR_pts_list),
list_summary_Discrete(cox_pts_list),
list_summary_Discrete(new_cox_pts_list)
)

colnames(COX_res)=c("CR","CAR","CARA","Proposed")
rownames(COX_res)=c("N11","N-11","N10","N-10","P1","P-1","Events","Z1_11","Z1_-11","Z1_10","Z1_-10","Z2_11","Z2_-11","Z2_10","Z2_-10")

write.xlsx(COX_res, file="/Users/renjieluo/Desktop/untitled folder/COX_res.xlsx",rowNames=T, overwrite = TRUE)
```
